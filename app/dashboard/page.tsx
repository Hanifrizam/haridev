...
type Portfolio = { id: number; title: string; type: string; color: string; status: string; link_url?: string };
...
export default function HariDevLanding() {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (element) { 
      ...
      setIsMenuOpen(false); // <-- INI YANG SUDAH DIPERBAIKI (Tidak pakai setIsMobileMenuOpen lagi)
    }
  };
    ...
    <div className="min-h-screen bg-[#020202] text-white font-sans selection:bg-cyan-500/30 scroll-smooth">
      ...
      <motion.nav style={{ opacity }} className="fixed top-0 left-0 right-0 z-[80] border-b border-white/5 bg-black/40 backdrop-blur-lg">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          {/* INI JUGA SUDAH DIPERBAIKI MENJADI setIsMenuOpen */}
            ...
            portfolios.map((item) => (
              <motion.div 
                ...
                onClick={() => {
                  ...
                  else handleDummyClick(`Portfolio: ${item.title}`);
                }}
              ...
              >
                {/* Render background image jika image_url ada */}
                {item.link_url && (
                  <div 
                    className="absolute inset-0 bg-cover bg-center opacity-40 group-hover:opacity-60 transition-opacity" 
                    style={{ backgroundImage: `url(${item.link_url})` }} 
                  />
                )}
